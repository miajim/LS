...
Inputs
  - integer
Outputs
  - string 'perfect', 'abundant', or 'deficient'
Rules/Req's
  - deficient: sum of divisors < number
  - perfect: sum of divisors = number
  - abundant: sum of divisors > number
  - input number must be > 1
Questions
  -
  -
Algorithm
  - create a class called PerfectNumber with ::classify
  - ::classify
    - guard clause: raise StandardError if input number < 1
    - from 1...number, select the numbers where num % current number == 0
    - sum the selected numbers
    - use if-elsif-else to compare sum to num and return appropriate string
=end
