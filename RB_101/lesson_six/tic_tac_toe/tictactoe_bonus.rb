...
def find_at_risk_square(brd)
  offense, defense = find_offensive_defensive_squares(brd)
...
def find_offensive_defensive_squares(brd)
  offense = []
  defense = []
  WINNING_LINES.each do |line|
    current_spaces = brd.values_at(*line)
    one_blank = (current_spaces.count(INITIAL_MARKER) == 1)
    if current_spaces.count(COMPUTER_MARKER) == 2 && one_blank
      offense << line[current_spaces.find_index(INITIAL_MARKER)]
    elsif current_spaces.count(PLAYER_MARKER) == 2 && one_blank
      defense << line[current_spaces.find_index(INITIAL_MARKER)]
    end
  end
