# RB101-RB109 Lesson 5
# Practice Problems: Sorting, Nested Collections, and Working with Blocks
# Problem 3
# Mon. 09/21/20

# --------------------------------------------------------------------------------------------------------------------------------------------------------------------
# LS Question
=begin
For each of these collection objects demonstrate how you would reference the letter 'g'.
=end
arr1 = ['a', 'b', ['c', ['d', 'e', 'f', 'g']]]

arr2 = [{first: ['a', 'b', 'c'], second: ['d', 'e', 'f']}, {third: ['g', 'h', 'i']}]

arr3 = [['abc'], ['def'], {third: ['ghi']}]

hsh1 = {'a' => ['d', 'e'], 'b' => ['f', 'g'], 'c' => ['h', 'i']}

hsh2 = {first: {'d' => 3}, second: {'e' => 2, 'f' => 1}, third: {'g' => 0}}

# --------------------------------------------------------------------------------------------------------------------------------------------------------------------
# My Solution
# Code
p arr1[2][1][3] == 'g'
p arr2[1][:third][0] == 'g'
p arr3[2][:third][0][0] == 'g'
p hsh1['b'][1] == 'g'
p hsh2[:third].key(0) == 'g'

# --------------------------------------------------------------------------------------------------------------------------------------------------------------------
# LS Solution Notes
=begin
Notes on the Hash#key method:
  - If more than one key has the same value, the method returns the key for the first occurrence of that value.
  - If the requested value does not exist in the hash, then nil is returned.
- Hash#keys (different method) would've been acceptable to use as well, b/c it returns an array of all
  the keys in the hash, and then you can reference values by index.
  e.g., hsh2[:third].keys[0] # => 'g'
=end

# --------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Addtional Notes
