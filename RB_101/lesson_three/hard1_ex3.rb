# RB101-RB109 Lesson 3
# Practice Problems - Hard 1: Question 3
# Sun. 09/06/20

# PROBLEM STATEMENT
=begin
In other practice problems, we have looked at how the scope of variables affects
the modification of one "layer" when they are passed to another.

To drive home the salient aspects of variable scope and modification of one
scope by another, consider the following similar sets of code.

What will be printed by each of these code groups?
=end

# A)

# def mess_with_vars(one, two, three)
#   one = two
#   two = three
#   three = one
# end

# one = "one"
# two = "two"
# three = "three"

# mess_with_vars(one, two, three)

# puts "one is: #{one}" # --> "one"
# puts "two is: #{two}" #--> "two"
# puts "three is: #{three}" #--> "three"

# B)

# def mess_with_vars(one, two, three)
#   # note that these string objects w/n the method are completely different
#   # from those outside of the method
#   one = "two"
#   two = "three"
#   three = "one"
# end

# one = "one"
# two = "two"
# three = "three"

# mess_with_vars(one, two, three)

# puts "one is: #{one}" # --> "one"
# puts "two is: #{two}" # --> "two"
# puts "three is: #{three}" # --> "three"

# C)

# method mutates the object pointed to by the outer local variables and the
# inner method variables
def mess_with_vars(one, two, three)
  one.gsub!("one","two")
  two.gsub!("two","three")
  three.gsub!("three","one")
end

one = "one"
two = "two"
three = "three"

mess_with_vars(one, two, three)

puts "one is: #{one}" # --> "two"
puts "two is: #{two}" #--> "three"
puts "three is: #{three}" #--> "one"

# CODE
# see comments w/n code above

# LS Solution
# (same)