...
def fun_with_ids
  ...
  # Notice that each vaiable keeps its new object/object_id even when we leave the block.
...
# CODE
# prediciting a_outer_id, etc. will be the same as a_outer.object_id even though a_outer_id, etc. weren't explicitly reassigned in the block
# predicting a_inner_id, etc. will be the same as a_outer_id, etc., however a_inner, etc. will not be available outside the block
# since it was initialized inside the block and thus will output an error, which is "rescued" by "ugh ohhhhh".

# LS Solution
# see comments above within the code for LS notes